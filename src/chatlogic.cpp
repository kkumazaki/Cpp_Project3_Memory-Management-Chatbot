...
ChatLogic::ChatLogic()
{
    ...
    //_chatBot = new ChatBot("../images/chatbot.png"); // Task 5: Moving the ChatBot

    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    //_chatBot->SetChatLogicHandle(this); // Task 5: Moving the ChatBot
...
ChatLogic::~ChatLogic()
{
    ...
    //delete _chatBot;  // Task 5: Moving the ChatBot

    // delete all nodes
    // Task 3: smart pointer doesn't need delete
    ...
    // delete all edges
    // Task 4: create a smart pointer in cpp, so remove here.
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> &node) { return node->GetID() == id; }); // Task 3: smart pointer

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            //_nodes.emplace_back(new GraphNode(id));
                            _nodes.emplace_back(std::make_unique<GraphNode>(id)); // Task 3: smart pointer
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                ...
                rootNode = it->get(); // Task 3: smart pointer
            }
            else
            {
